...
def _extract_product_meta(soup: BeautifulSoup) -> tuple[str, str, str | None, str | None, str | None]:
    """
    Extract (product_name, product_brand, product_image, product_shade, product_category)
    from page 1 HTML. Returns safe defaults when elements are not found.
    ...
    # Breadcrumb category — cari item yang mengandung kata kunci kategori lip product
    breadcrumb_items = soup.select('[class*="breadcrumb-section-text"]')
    product_category = None
    _lip_keywords = ('lip', 'blush', 'eye', 'foundation', 'tint', 'gloss', 'balm', 'stain', 'liner', 'plumper', 'serum', 'cream', 'stick')
    for item in breadcrumb_items:
        text = item.get_text(strip=True)
        if text and any(kw in text.lower() for kw in _lip_keywords):
            product_category = " ".join(text.split())
            break

    print(f"[Scraper] Brand   : {product_brand!r}")
    print(f"[Scraper] Name    : {product_name!r}")
    print(f"[Scraper] Shade   : {product_shade!r}")
    print(f"[Scraper] Category: {product_category!r}")
    return product_name, product_brand, product_image, product_shade, product_category
    ...
    async with async_playwright() as pw:
        ...
        try:
            for page_num in range(1, MAX_PAGES + 1):
                ...
                if page_num == 1:
                    product_name, product_brand, product_image, product_shade, product_category = _extract_product_meta(soup)
                    print(f"[Scraper] Product: {product_brand} – {product_name} (shade: {product_shade})")

                    # URL-based fallback when breadcrumb extraction returns nothing
                    # URL format new: /products/lips/lip-balm-treatments/brand/product → index 2
                    # URL format old: /lip/lipstick/brand/product                     → index 1
                    if not product_category:
                        try:
                            path = url.split("reviews.femaledaily.com")[-1]
                            ...
                            if len(path_parts) >= 3 and path_parts[0] == "products":
                                ...
                                slug = None
                            if slug:
                                product_category = slug.replace("-", " ").title()
                                print(f"[Scraper] Category (URL fallback): {product_category!r}")
                        except Exception:
                            pass
